...
ri <- ri_data %>%
  ...
  mutate(ri_threshold = case_when(average_cov < 83 ~ 0,      # Add a binary flag to signal if the average DTP3 coverage is > 83 since we see a different relationship after this point
                                   .default = 1))
...
ri_combo_dat <- tracker %>%
  select(iso_code, average_cov, ri_threshold, delta_2020, delta_2021, delta_2022) %>%
  pivot_longer(!(iso_code:ri_threshold), values_to = "delta", names_to = "year", names_prefix = "delta_")
...
ri_per_year <- ggplot(data = tracker)+
  ...
  theme(legend.position = c(0.2,0.2), 
        ...
        axis.title = element_text(size = 8))+
  hline+
  geom_vline(xintercept = 0.83, linetype = "dashed", colour = "purple")
...
#### Univariate model
```{r}
lm_ri <- lm(data = tracker_combo, formula = delta ~ average_cov * ri_threshold)
summary(lm_ri)
```
...
hcw %<>%
  mutate(healthworkforce_threshold = case_when(healthworkforce < 50 ~ 0,
                                               .default = 1)) %>%
  select(iso_code, healthworkforce, healthworkforce_threshold)
...
healthworkforce_combo_dat <- tracker %>%
  select(iso_code, healthworkforce, healthworkforce_threshold, delta_2020, delta_2021, delta_2022) %>%
  pivot_longer(cols = !c(iso_code:healthworkforce_threshold), names_prefix = "delta_", names_to = "year", values_to = "delta")
...
healthworkforce_per_year <- ggplot(data = tracker)+
  ...
  theme+
  hline+
  geom_vline(xintercept = 50, linetype = "dashed", colour = "purple")
...
#### Univariate model
```{r}
lm_hwf <- lm(data = tracker_combo, formula = delta ~ healthworkforce * healthworkforce_threshold)
summary(lm_hwf)
...
uhc %<>%
  ...
  mutate(uhc_threshold = case_when(uhc_mean_index < 70 ~ 0,      # Add a binary flag to signal if the average UHC index is > 70 since we see a different relationship after this point
                                   .default = 1))
...
uhc_combo_dat <- tracker %>%
  select(iso_code, uhc_mean_index, uhc_threshold, delta_2020, delta_2021, delta_2022) %>%
  pivot_longer(!(iso_code:uhc_threshold), values_to = "delta", names_to = "year", names_prefix = "delta_")
...
uhc_scatter_per_year <- ggplot(data = tracker)+
  ...
  theme+
  hline+
  geom_vline(xintercept = 70, linetype = "dashed", colour = "purple")
...
#### Univariate model
```{r}
lm_uhc <- lm(data = tracker_combo, formula = delta ~ uhc_mean_index * uhc_threshold)
summary(lm_uhc)
...
#### Simplify data
```{r}
# Select 2019 Global health security assessment; and add threshold for change in relationship point
ghs %<>%
  select(iso_code, ghs_index = ghs_index_2019) %>%
  mutate(ghs_threshold = case_when(ghs_index < 35 ~ 0,      # Add a binary flag to signal if the average GHS score under 35 since we see a different relationship after this point
                                   .default = 1))
...
ghs_combo_dat <- tracker %>%
  select(iso_code, ghs_index, ghs_threshold, delta_2020, delta_2021, delta_2022) %>%
  pivot_longer(cols = !c(iso_code:ghs_threshold), names_prefix = "delta_", names_to = "year", values_to = "delta")
...
ghs_per_year <- ggplot(data = tracker)+
  ...
  theme+
  hline+
  geom_vline(xintercept = 35, linetype = "dashed", colour = "purple")
...
#### Univariate model
```{r}
lm_ghs <- lm(data = tracker_combo, formula = delta ~ ghs_index * ghs_threshold)
summary(lm_ghs)
...
vx_total <- ggplot(data = tracker %>% filter(iso_code != "VNM"))+
  ...
  theme+
  hline+
  geom_vline(xintercept = 100000, colour = "purple", linetype = "dashed")
...
gdp <- gdp_data %>%
  ...
  mutate(gdp_threshold = case_when(average_gdp >= 15000 ~ 1,
                                   .default = 0))
...
gdp_combo_dat <- tracker %>%
  select(iso_code, average_gdp, gdp_threshold, delta_2020, delta_2021, delta_2022) %>%
  pivot_longer(cols = !c(iso_code:gdp_threshold), names_prefix = "delta_", names_to = "year", values_to = "delta")
...
gdp_scatter_per_year <- ggplot(data = tracker)+
  ...
  geom_vline(xintercept = 15000, linetype = "dashed", colour = "purple")+
  colour+
  scale_y_lim+
  theme+
  hline
...
#### Univariate model
```{r}
lm_gdp <- lm(data = tracker_combo, formula = delta ~ average_gdp * gdp_threshold)
summary(lm_gdp)
...
# Output per policy
containment_plots <- plot_grid(containment_scatter, containment_combo, labels = c("A", "B", ncol = 2))
economic_plots <- plot_grid(economic_scatter, economic_combo, labels = c("A", "B", ncol = 2))
health_plots <- plot_grid(health_scatter1, health_combo1, health_scatter2, health_combo2, labels = c("1A", "1B", "2A", "2B", ncol = 2, nrow = 2))
  ...
  # Outside
    outside_per_year <- ggplot(data = tracker)+
      ...
      theme(legend.position = c(0.8,0.2),
        ...
        axis.title = element_text(size = 8))+
      hline+
      geom_vline(xintercept = -1.5, linetype = "dashed", colour = "purple")
...
# Output 
mobility_per_year <- plot_grid(outside_per_year, inside_per_year, 
                              labels = c("Outside", "Inside"), 
                              ncol = 1, nrow = 2,
                              label_size = 8)
mobility_combo <- plot_grid(outside_combo, inside_combo,
                            labels = c("Outside", "Inside"), 
                            ncol = 1, nrow = 2,
                            label_size = 8)

outside_plots <- plot_grid(outside_per_year, outside_combo,
                           labels = c("A", "B"),
                           label_size = 10)

residential_plots <- plot_grid(inside_per_year, inside_combo,
                               labels = c("A", "B"),
                               label_size = 10)
...
completeness <- data.frame(Column = names(completeness_table), NonNA_Count = completeness_table) %>%
  mutate(percent = round((NonNA_Count/182)*100,0))
completeness
...
### Vaccine hesitancy data exploration
...
#### Vaccine Confidence Project data
- Source: Vaccine Confidence Project
- Here we investigate and describe the limited data on vaccine hesitancy, which is used in the discussion section of the paper only
```{r}
file_path_confidence <- here::here("data", "vcp-global-data.rds")
  ...
  # Split into pre- and during-pandemic datasets
  ...
  # For pre-pandemic take the most recent year of data for each country
  confidence_pre %<>% 
    group_by(country) %>%
    filter(year == max(year)) %>%
    ungroup() # Data for 78 countries
  
  confidence_during %<>% 
    group_by(country) %>%
    filter(year == max(year)) %>%
    ungroup() # Data for 86 countries
```

#### Facebook vaccine hesitancy data
- Source: Facebook dataforgood
- Description: Facebook collected data from users on vaccine hesitancy with regards to COVID-19 vaccination during the pandemic. These surveys provide an indication of vaccine hesitancy for adults, but do not necessarily indicate vaccine hesitancy with regards to immunising their children (e.g., a parent may still take a child for routine vaccinations but have concerns around a faster developed COVID-19 vaccination for themselves). This analysis is commented out (since it takes a while to download the dataset, and review determined it was insufficiently complete to use for this analysis. It is included in case others would like to consider it's applicability to adjacent analyses)

```{r}
# Vx barrier reason data
  #facebook_path <- "https://covidmap.umd.edu/api/resources?indicator=barrier_reason_dontlike&type=smoothed&country=all&daterange=20200315-20211231"
  #request <- GET(url = facebook_path)
  #response <- content(request, as = "text", encoding = "UTF-8")
  #vx_hesitancy <- fromJSON(response, flatten = TRUE) %>% data.frame()
  #view(vx_hesitancy)
```
...
```{r}
# Final dataset
complete_data <- tracker_combo %>%
  na.omit() %>%
  select(-contains("threshold")) %>% # Remove threshold indicators as these will be set - if needed - by residuals 
as_tibble()
```
...
# Add back in ri_threshold indicator
  complete_data %<>%
    mutate(ri_threshold = case_when(average_cov < 83 ~ 0,      # Add a binary flag to signal if the average DTP3 coverage is > 83 since we see a different relationship after this point
                                   .default = 1))

  model1 <- lm(data = complete_data, formula = delta ~ average_cov)
  summary(model1)
# Linear model
  model1a <- lm(data = complete_data, formula = delta ~ average_cov * ri_threshold)
  summary(model1a)
    # Include only the RI threshold and interaction term; remove average_cov
  
  model1b <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov)
  summary(model1b)
...
# Set-up data for plotting next variable
  plot_dat <- complete_data 
  model1res <- model1b$residuals
  plot_dat %<>% cbind(model1res)
```

### 2) Health workforce capacity
```{r}
# Linear model
  model2 <- lm(data = complete_data,
                    formula = delta ~ ri_threshold + ri_threshold:average_cov + healthworkforce)
  summary(model2)
  
# Visualise relationship
  model2_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = healthworkforce, y = model1res))+
    geom_smooth(aes(x = healthworkforce, y = model1res), colour = "darkgrey")
  model2_plot
    # Looks like different relationship before and after healthworkforce =~ 60

  complete_data %<>%
    mutate(healthworkforce_threshold = case_when(healthworkforce < 60 ~ 0,
                                                 .default = 1))
  
  model2b <- lm(data = complete_data,
...
# Compare AIC of models
  ...
  # Set-up data for plotting next variable
  model2res <- model2c$residuals
  plot_dat %<>% cbind(model2res)
```

### 3) Health systems strength
```{r}
# Linear model
  model3 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov+
                       healthworkforce + healthworkforce_threshold:healthworkforce + 
                      uhc_mean_index)
  summary(model3)
  
# Visualise relationship
  model3plot <- ggplot(data = complete_data)+
    geom_point(aes(x = uhc_mean_index, y = model2res)) + 
    geom_smooth(aes(x = uhc_mean_index, y = model2res), colour = "darkgrey")+
    theme
  model3plot  
...
```{r}
# Linear model
  model4 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov+
...
# Visualise relationship
  model4plot <- ggplot(data = complete_data)+
    geom_point(aes(x = ghs_index, y = model2res)) + 
    geom_smooth(aes(x = ghs_index, y = model2res), colour = "darkgrey")+
    theme
  model4plot  
    # No linear or non linear relationship. Do not add GHS index
```

### 5) Covid-19 vaccination
```{r}
# Linear model
  model5 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov+
                       healthworkforce + healthworkforce_threshold:healthworkforce + vx)
  summary(model5)
  BIC(model5)
  
# Visualise relationship
  model5_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = vx, y = model2res))+
    geom_smooth(aes(x = vx, y = model2res), colour = "darkgrey")
  model5_plot
...
```{r}
# Linear model
  model6 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov+
                       healthworkforce + healthworkforce_threshold:healthworkforce + mortality)
  summary(model6)
  BIC(model6)
  
# Visualise relationship
  model6_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = mortality, y = model2res))+
    geom_smooth(aes(x = mortality, y = model2res), colour = "darkgrey")
  model6_plot
    # Linear
  
# Calculate BIC
  BIC(model2c, model6)
    # Do not include mortality (but borderline)
```

### 7) Health policy during COVID-19
```{r}
# Linear model
  model7 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov + 
...
# Visualise relationship
  model7a_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = healthPC1, y = model2res))+ 
    geom_smooth(aes(x = healthPC1, y = model2res), colour = "darkgrey")
  model7a_plot
  
  model7b_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = healthPC2, y = model2res))+ 
    geom_smooth(aes(x = healthPC2, y = model2res), colour = "darkgrey")
  model7b_plot
...
```{r}
# Linear model
  model8 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov+
                       healthworkforce + healthworkforce_threshold:healthworkforce + gov)
  summary(model8)
  BIC(model8)
  
# Visualise relationship
  model8_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = gov, y = model2res))+
    geom_smooth(aes(x = gov, y = model2res), colour = "darkgrey")
  model8_plot
    # No clear linear or non linear relationship
```
...
```{r}
# Linear model
  model9 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov + 
                      healthworkforce + healthworkforce_threshold:healthworkforce + ext)
  summary(model9)
  BIC(model9)
  
# Visualise relationship
  model9_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = ext, y = model2res))+
    geom_smooth(aes(x = ext, y = model2res), colour = "darkgrey")
  model9_plot
    # No clear linear or non linear relationship --> Do not include external financing
```

#### 10) Private healthcare financing
```{r}
# Linear model
  model10 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov + 
                      healthworkforce + healthworkforce_threshold:healthworkforce + private)
  summary(model10)
  BIC(model10)
  
# Visualise relationship
  model10_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = private, y = model2res))+
    geom_smooth(aes(x = private, y = model2res), colour = "darkgrey")
  model10_plot
    # No clear linear or non linear relationship. Few outliers at 5000 private financing that is slightly skewing the trend
...
```{r}
# Linear model
  model11 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov + 
                      healthworkforce + healthworkforce_threshold:healthworkforce + outside)
  summary(model11)

# Visualise relationship
  model11_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = outside, y = model2res))+
    geom_smooth(aes(x = outside, y = model2res), colour = "darkgrey")
  model11_plot

# Compare AIC
  BIC(model2c, model11)
    # BIC is slightly lower when no mobility terms included therefore do not include, noting mobility associated with delta with p < 0.023 after previously included terms
```

#### 12) Inside
```{r}
# Add inside mobility
  model12 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov + 
                      healthworkforce + healthworkforce_threshold:healthworkforce + inside)
  summary(model12)
  BIC(model12)
  
# Visualise relationship
  model12_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = inside, y = model2res))+
    geom_smooth(aes(x = inside, y = model2res), colour = "darkgrey")
  model12_plot
...
```{r}
# Linear model
  model13 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov + 
...
# Visualise relationship
  model13_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = containment, y = model2res))+
    geom_smooth(aes(x = containment, y = model2res), colour = "darkgrey")
  model13_plot
  # Do not include containment policy (not quite linear but not much variation)
```
#### 14) Economic
```{r}
# Linear model
  model14 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov + 
...
# Visualise relationship
  model14_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = economic, y = model2res))+ 
    geom_smooth(aes(x = economic, y = model2res), colour = "darkgrey")
  model14_plot
    # Do not include economic policy
```
### 15) GDP
```{r}
# Linear model
  model15 <- lm(data = complete_data, formula = delta ~ ri_threshold + ri_threshold:average_cov + 
...
# Visualise relationship
  model15_plot <- ggplot(data = plot_dat)+
    geom_point(aes(x = average_gdp, y = model2res))+ 
    geom_smooth(aes(x = average_gdp, y = model2res), colour = "darkgrey")
  model15_plot
    # Do not include GDP
```
...
model_summary <- tidy(model_complete_final) %>%
  ...
  mutate(Term = c("Intercept", "Threshold coverage (binary variable): Pre-pandemic\n DTP3 coverage > 83%", "Health workforce capacity", "Threshold coverage * pre-pandemic DTP3 coverage",
                  "Threshold health workforce (binary variable; > 60 health workers per 10k population) * health workforce capacity"),
         p.value = ifelse(p.value == 0, 
                          "<0.00001", 
                          format(p.value, scientific = FALSE, digits = 5))) %>%
...
print(doc, target = "../figures/lm_summary.docx")
...
# Explore residuals
  ...
  # Combine dataset
  residuals_explore <- complete_data %>%
...
```{r}
random_forest_data <- complete_data %>%
  select(-contains("threshold"))
  # Remove the underlying mobility variables as using 'outside' only to reduce correlations
...
train_data <- random_forest_data[train_indices, ] 
test_data <- random_forest_data[-train_indices, ] 
...
importance_data %<>%
  ...
  ggsave(importance_plot, 
         filename = "../figures/rf_importance.png",
...
# Plot coverage deltas visually
  ...
  # Format data for plotting  
    map_tracker <- tracker %>%
        mutate(average_containment = apply(select(., containment_PC1_2020,
                                                  containment_PC1_2021,
                                                  containment_PC1_2022), 1, mean, na.rm = TRUE),
              average_health_PC1 = apply(select(., health_PC1_2020, 
                                                health_PC1_2021, 
                                                health_PC1_2022), 1, mean, na.rm = TRUE),
              average_health_PC2 = apply(select(., health_PC2_2020, 
                                                health_PC2_2021, 
                                                health_PC2_2022), 1, mean, na.rm = TRUE),
              average_economic = apply(select(., economic_PC1_2020, 
                                              economic_PC1_2021, 
                                              economic_PC1_2022), 1, mean, na.rm = TRUE),
              average_outside = apply(select(., outside_2020, 
                                             outside_2021, 
                                             outside_2022), 1, mean, na.rm = TRUE),
              average_residential = inside_2020 + inside_2021 + inside_2022) %>%
      select(iso_a3 = iso_code, delta_total, average_cov, healthworkforce, uhc_mean_index,
             ghs_index, covid_vx_total, excess_mortality_total, 
             average_containment, average_health_PC1, average_health_PC2, 
             average_economic, gov, ext, private, average_outside, average_residential, 
             average_gdp) %>%
  ...
  # Plot COVID-19 vaccination
   ...
   # Plot Containment score
    containment_map <- ggplot(data = map_data) +
      geom_sf(aes(fill = average_containment), color = "black", size = 0.2) + 
      scale_fill_gradientn(colours = pal,
                           na.value = "grey80",
                           guide = guide_colorbar(reverse = FALSE), 
                           labels = comma) + 
  ...
  # Plot Economic score
    economic <- ggplot(data = map_data) +
      geom_sf(aes(fill = average_economic), color = "black", size = 0.2) + 
      scale_fill_gradientn(colours = rev(pal),
                           na.value = "grey80",
                           guide = guide_colorbar(reverse = FALSE), 
                           labels = comma) + 
  ...
  # Plot Health policy score
    health_map1 <- ggplot(data = map_data) +
      geom_sf(aes(fill = average_health_PC1), color = "black", size = 0.2) + 
      scale_fill_gradientn(colours = rev(pal),
                           na.value = "grey80",
                           guide = guide_colorbar(reverse = FALSE), 
                           labels = comma) + 
    ...
    health_map2 <- ggplot(data = map_data) +
      geom_sf(aes(fill = average_health_PC2), color = "black", size = 0.2) + 
      scale_fill_gradientn(colours = rev(pal),
                           na.value = "grey80",
                           guide = guide_colorbar(reverse = FALSE), 
                           labels = comma) + 
...
# Private health financing
  ...
  # Plot outside mobility 
    outside_map <- ggplot(data = map_data) +
      geom_sf(aes(fill = average_outside), color = "black", size = 0.2) + 
      scale_fill_gradientn(colours = rev(pal),
                           na.value = "grey80",
                           guide = guide_colorbar(reverse = FALSE), 
                           labels = comma) + 
  ...
  # Plot inside mobility 
    inside_map <- ggplot(data = map_data) +
      geom_sf(aes(fill = average_residential), color = "black", size = 0.2) + 
      scale_fill_gradientn(colours = rev(pal),
                           na.value = "grey80",
                           guide = guide_colorbar(reverse = FALSE), 
                           labels = comma) + 
